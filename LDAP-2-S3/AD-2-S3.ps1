...
try
    {
        ...
        Get-ADUser -Filter {Enabled -eq $true} -properties DisplayName, EmailAddress, OfficePhone| select DisplayName, EmailAddress, OfficePhone | ConvertTo-Json | New-Item $env:temp\zendesk_sync\active_users\$DomainName.json -type file -force
